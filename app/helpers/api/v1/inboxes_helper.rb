module Api::V1::InboxesHelper
  def inbox_name(channel)
    if channel.is_a?(Channel::Telegram)
      ...
      # O bot_name é definido no before_validation do modelo Channel::Telegram
      bot_name = channel.bot_name
      result = bot_name.presence || permitted_params[:name]
      Rails.logger.info "[InboxesHelper] Telegram channel bot_name: #{bot_name.inspect}, permitted_params[:name]: #{permitted_params[:name].inspect}, returning: #{result.inspect}"
      result
    else
