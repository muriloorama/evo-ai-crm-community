...
class Integrations::Facebook::MessageCreator
  ...
  def agent_message_via_echo?
    # TODO : check and remove send_from_evolution_app if not working
    response.echo? && !response.sent_from_evolution_app?
    # this means that it is an agent message from page, but not sent from Evolution.
  ...
  def create_agent_message
    Channel::FacebookPage.where(page_id: response.sender_id).each do |page|
      Accountable.with_account(page.account_id) do
